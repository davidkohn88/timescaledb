...
chunk_dispatch_exec(CustomScanState *node)
{
	...
	if (!TupIsNull(slot))
	{
		...
		{
		   /* In PG11 several fields were removed from the ModifyTableState node and
			* ExecInsert function nodes, as they were redundant. (See:
...
chunk_dispatch_state_set_parent(ChunkDispatchState *state, ModifyTableState *parent)
{
	 ...
	 * it's accessing a normal table (and not when it's accessing a partitioned
	 * table) we make copies of these slots and replace them so that we can
	 * modify the tupledesc later in our code.
	*/
	...
	if (parent->mt_conflproj != NULL)
	{
		...
		/*
		 * in this case we must overwrite mt_conflproj because there are several
		 * pointers to it throughout expressions and other evaluations, and the
		 * original tuple will otherwise be stored to the old slot, whose
		 * pointer is saved there.
		 */
