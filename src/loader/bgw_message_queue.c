...
typedef struct tsbgwMessageQueue{
    pid_t                  reader_pid; /* should only be set once at cluster launcher startup*/
    LWLock                  *lock; /*pointer to the lock to control adding/removing elements from queue*/
    int                     read_upto;
    int                     num_elements;
...
static bool tsbgw_message_queue_add(tsbgwMessageQueue *queue, tsbgwMessage *message){
    ...
    if (reader_pid != 0)/*set latch on the reader as long as one is defined, even if queue is full, then we def need to read messages */
