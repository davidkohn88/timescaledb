...
extern bool timescale_bgw_increment_total_workers(int max_workers)  {
    ...
    if (ss->total_workers < max_workers) /* result can be <= max_workers, so we test for less than*/
    {
        ss->total_workers++;
        incremented = true;
    }
 ...
 */
static HTAB  *populate_database_htab(void){
    Relation        rel;
    HeapScanDesc    scan;
    HeapTuple       tup;
    HTAB            *db_htab = NULL;
    HASHCTL         info;
    /* first initialize the hashtable before we start a txn, we'll be writing info about dbs there */
    info = (HASHCTL){
        .keysize = sizeof(Oid),
...
static void start_db_schedulers(HTAB *db_htab){
...
static void check_for_stopped_db_schedulers(HTAB *db_htab){
    HASH_SEQ_STATUS         hash_seq;
    tsbgwHashEntry          *current_entry;
    List                    *dead_oids = NIL;
    ListCell                *lc;
...
static void stop_all_db_schedulers(HTAB *db_htab){
    HASH_SEQ_STATUS         hash_seq;
    tsbgwHashEntry          *current_entry;
    hash_seq_init(&hash_seq, db_htab);
 ...
 */
static void launcher_handle_messages(HTAB *db_htab){
...
extern void timescale_bgw_cluster_launcher_main(void) {
...
extern void timescale_bgw_db_scheduler_entrypoint(Oid db_id){
    ...
    /* @Mat for safety we should call extension_check() here, but unclear how to call that now*/
    CommitTransactionCommand();
	if (ts_installed)
	{
 ...
 */
Datum timescale_bgw_restart_db_workers(PG_FUNCTION_ARGS){
    tsbgwMessage            *message;
    bool                    success;
