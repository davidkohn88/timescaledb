...
static void
bgw_counter_state_init(bool reinit)
{
	bool		found;
...
{
	ereport(DEBUG1, (errmsg("In counter_shmem_startup")));
	bgw_counter_state_init(false);
}

extern void
bgw_counter_shmem_cleanup(void)
{
	/*Leave our state nice for when we restart*/
	bgw_counter_state_init(true);
}
