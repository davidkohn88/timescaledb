...
/*
 * Before we morph into the scheduler, we also need to reload configs from
 * their defaults if the database default has changed (to, for instance, pick
 * up if we have changed the timescaledb.restoring flag). This function is
 * based on the postgres function in postinit.c by the same name.
 */
