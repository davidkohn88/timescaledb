...
static void
chunk_insert_state_translate_arbiter_indexes(ChunkInsertState *state, ChunkDispatch *dispatch, Relation chunk_rel)
{
	ListCell   *lc;
...
chunk_insert_state_create(Chunk *chunk, ChunkDispatch *dispatch)
{
	...
	OnConflictAction onconflict = dispatch->on_conflict;
	...
	if (onconflict != ONCONFLICT_NONE)
		chunk_insert_state_translate_arbiter_indexes(state, dispatch, rel);
