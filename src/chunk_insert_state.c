...
ts_chunk_insert_state_destroy(ChunkInsertState *state)
{
	...
	MemoryContextCallback *free_callback_ptr;
	...
	free_callback_ptr = MemoryContextAlloc(state->estate->es_query_cxt, sizeof(*free_callback_ptr));
	*free_callback_ptr = free_callback;
	MemoryContextRegisterResetCallback(deletion_context, free_callback_ptr);
